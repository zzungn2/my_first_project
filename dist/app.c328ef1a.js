...
var ajax = new XMLHttpRequest(); // newsFeed

var NEWS_URL = 'http://api.hnpwa.com/v0/news/1.json'; // newsContent

var CONTENT_URL = 'https://api.hnpwa.com/v0/item/@id.json'; // title이 표현되는 부분

var content = document.createElement('div');
var container = document.getElementById('root'); // 공유 값/
...
function newsFeed() {
  var newsFeed = store.feeds; // 배열 

  var newsList = []; // template
...
function newsDetail() {
  ...
  // 데이터 열기, 불러오기, 처리
  // @제외, #제외 
  var id = location.hash.substr(7); // ajax.open('GET', CONTENT_URL.replace('@id', id), false);
  // ajax.send();
  // const newsContent = JSON.parse(ajax.response);

  var newsContent = getData(CONTENT_URL.replace('@id', id)); // h1으로 표현
